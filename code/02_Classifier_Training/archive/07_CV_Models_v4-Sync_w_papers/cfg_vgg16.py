...
EPOCHS = 100
BATCH_SIZE = 64 #128 # good w/ LR0.01 and great w/ LR0.005 #32
STEPS_PER_EPOCH_TRAIN = -1
STEPS_PER_EPOCH_VAL = -1

MODEL_OPT = "SGD"

# 0.005 - Even better w/ 64 batch-size #0.01 - Good w/ no decay #1.0e-4
# Batch_Size = 64
# LR = 0.001
# LR_Decay_steps = 10000/Batch_Size*2 = 312 (~3 epochs)

MODEL_LR = 0.001
lr_schedule = ExponentialDecay(
                            initial_learning_rate=MODEL_LR,
                            decay_steps=TOTAL_NUMBER_OF_IMAGES/BATCH_SIZE*2,
                            decay_rate=0.95,
                            staircase=True)

MODEL_OPTIMIZER = keras.optimizers.SGD(learning_rate=lr_schedule,
                                       momentum=0.9)
