...
def run_logic(CASE: str, MODEL: str):
    dir_to_results = "C:\\Users\\johns\\PycharmProjects\\VT\\" \
                     "training_cv_models_on_ue_images_jws\\code\\" \
                     "02_Classifier_Training\\output\\" \
                     f"{MODEL}\\{CASE}"
    output_file = "C:\\Users\\johns\\PycharmProjects\\VT\\" \
                     "training_cv_models_on_ue_images_jws\\code\\" \
                     "02_Classifier_Training\\output\\" \
                     f"\\{MODEL}\\{CASE}_test_results.csv"
    ...
    for dir in os.listdir(dir_to_results):
        ...
        for file in os.listdir(abs_path):
            if "REF_run_info" in file:
                ...
                _quantities['id'] = abs_path.split('-')[-1]

                if len(str(_quantities['id'])) == 1:
                    _quantities['id'] = "0" + _quantities['id']
...
if __name__=="__main__":
    ...
    for model in MODEL:
        for case in CASE:
            run_logic(CASE=case, MODEL=model)